...
pub fn derive_message(item: TokenStream) -> Result<TokenStream, Error> {
    ...
    Ok(quote! {
        impl #impl_generics ::polybox::messaging::Message for #ident #ty_generics #where_clause {
            type Returned = <#msg_type as ::polybox::messaging::MessageDerive<Self>>::Returned;
            type Payload = <#msg_type as ::polybox::messaging::MessageDerive<Self>>::Payload;
            fn create(self) -> (Self::Payload, Self::Returned) {
                <#msg_type as ::polybox::messaging::MessageDerive<Self>>::create(self)
            }
            fn cancel(payload: Self::Payload, returned: Self::Returned) -> Self {
                <#msg_type as ::polybox::messaging::MessageDerive<Self>>::cancel(payload, returned)
            }
        }
    })
}
...
pub fn get_msg_type(attrs: &Vec<Attribute>) -> Result<TokenStream, Error> {
    ...
    if let Some(attr) = attrs.iter().find(|attr| attr.path.is_ident("request")) {
        ...
        let ty = attr.parse_args::<Type>()?;
        msg_type = Some(quote! { ::polybox::messaging::Rx<#ty> })
    };
