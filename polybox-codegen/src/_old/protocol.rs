...
fn impl_accepts(item: &ItemEnum, variants: &Vec<ProtocolMsg>) -> Result<TokenStream, Error> {
    ...
    let accepts = variants.iter().map(|variant| {
            ...
            quote! {
                impl #impl_generics ::polybox::messaging::FromPayload<#variant_ty> for #ident #ty_generics #where_clause {
                    fn from_payload(
                        msg: <#variant_ty as ::polybox::messaging::Message>::Payload
                    ) -> Self {
                        Self::#variant_ident(msg)
                    }

                    fn try_into_payload(self) -> Result<
                        <#variant_ty as ::polybox::messaging::Message>::Payload,
                        Self
                    > {
                        match self {
...
fn impl_protocol(item: &ItemEnum, variants: &Vec<ProtocolMsg>) -> Result<TokenStream, Error> {
    ...
    let matches = variants
        ...
        .map(|variant| {
            ...
            quote! {
                Self::#variant_ident(msg) => {
                    ::polybox::messaging::BoxPayload::new::<#variant_ty>(msg)
                }
            }
        })
        .collect::<Vec<_>>();
    ...
    Ok(quote! {
        impl #impl_generics ::polybox::messaging::Protocol for #ident #ty_generics #where_clause {

            fn try_from_boxed_payload(boxed: ::polybox::messaging::BoxPayload) -> Result<Self, ::polybox::messaging::BoxPayload> {
                #(#downcasts)*
                Err(boxed)
            }
            ...
            fn into_boxed_payload(self) -> ::polybox::messaging::BoxPayload {
                match self {
                    #(#matches)*
                }
            }
...
fn impl_handled_by(item: &ItemEnum, variants: &Vec<ProtocolMsg>) -> Result<TokenStream, Error> {
    let new_generics = {
        ...
        new_generics
            ...
            .predicates
            .push(parse_quote! {
                H: ::polybox::handler::Handler + #handle_msg_traits
            });
        new_generics
    };
    ...
    Ok(quote! {
        ...
        impl #impl_generics ::polybox::handler::HandledBy<H> for #ident #ty_generics #where_clause {
            async fn handle_with(
                self,
                handler: &mut H,
                state: &mut H::State,
            ) -> Result<::polybox::handler::Flow<H>, H::Exception> {
                match self {
                    #(#matches)*
                }
            }
            ...
            if let Fields::Unnamed(fields) = &mut variant.fields {
                if fields.unnamed.len() == 1 {
                    ...
                    fields.unnamed.push(Field {
                        ...
                        colon_token: None,
                        ty: parse_quote! {
                            <#ty as ::polybox::messaging::Message>::Payload
                        },
                    });
