...
//     channel.channel_data().set_status(ActorStatus::Running);
//     channel
// }
...
//     // Empty initially.
//     assert_matches!(channel.channel_data().pop_signal(), None);

//     // Synchronous push/pop.
//     channel.signal_shutdown();

//     assert_matches!(channel.channel_data().pop_signal(), Some(_));
//     assert_matches!(channel.channel_data().pop_signal(), None);

//     // Async receive.
//     let channel_clone = channel.clone();

//     let handle = tokio::spawn(async move { channel_clone.channel_data().recv_signal().await });
...
//     let received = channel.channel_data().recv_msg().await;
...
//     assert!(channel.channel_data().pop_msg().is_none());
// }
...
//     assert_eq!(Arc::as_ptr(&channel.data), Arc::as_ptr(&clone.data),);
// }
...
//     let original_ptr = Arc::as_ptr(&channel.data);

//     let dyn_channel = channel.into_dyn_unchecked::<Set!()>();

//     let erased_ptr = Arc::as_ptr(&dyn_channel.data);
...
//     assert_eq!(original_ptr, Arc::as_ptr(&dyn_channel.data));
//     assert_eq!(original_ptr, Arc::as_ptr(&typed_channel.data));
// }
...
//     assert_eq!(original_ptr, Arc::as_ptr(&restored.data));
// }
...
//     assert_eq!(original_ptr, Arc::as_ptr(&original.data));
...
//     assert_eq!(original_ptr, Arc::as_ptr(&channel.data));
// }
